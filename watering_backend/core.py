...
from watering_backend.database import connection as database_connection
from watering_backend.forecast import simulate_tanks
from watering_backend.home_assistant import post_webhook
from watering_backend.http_api import serve as serve_http_api
from watering_backend.notifications import NotificationService, NotificationWorker, SMTPConfig
...
from watering_backend.schema import migrate as migrate_backend_schema
from watering_backend.scheduling import watering_times as configured_distributed_windows
...
def connect():
    return database_connection(DATA_DIR, DB_PATH)
...
def planner_config() -> dict:
    raw = get_setting("planner_config", "")
    saved: dict = {}
    if raw:
        try:
    ...
    if "refill_windows" not in saved:
        legacy_raw = get_setting("refill_schedule_times", json.dumps(list(REFILL_RUN_TIMES)))
        try:
            legacy_value = json.loads(legacy_raw)
        except json.JSONDecodeError:
            legacy_value = legacy_raw
        legacy_times = normalize_refill_schedule_times(legacy_value)
        saved["refill_windows"] = [
            {
                "start": item,
                "end": (
                    datetime.combine(date.today(), parse_hhmm(item)) + timedelta(minutes=REFILL_TRIGGER_TOLERANCE_MINUTES)
                ).strftime("%H:%M"),
            }
    ...
    if "refill_min_interval_minutes" not in saved:
        saved["refill_min_interval_minutes"] = REFILL_MIN_INTERVAL_MINUTES
    return validate_planner_config(saved)


def save_planner_config(value: object) -> dict:
    config = validate_planner_config(value)
    set_setting("planner_config", json.dumps(config, separators=(",", ":"), sort_keys=True))
    # Keep the old fields readable during the compatibility period.
    set_setting("refill_schedule_times", json.dumps([item["start"] for item in config["refill_windows"]]))
    return config
...
def save_refill_schedule_times(value: object) -> None:
    times = normalize_refill_schedule_times(value)
    config = planner_config()
    config["refill_windows"] = []
    for item in times:
        start = datetime.combine(date.today(), parse_hhmm(item))
        end = start + timedelta(minutes=REFILL_TRIGGER_TOLERANCE_MINUTES)
        if end.date() != start.date():
            raise ValueError("Nachfuellzeitfenster duerfen nicht ueber Mitternacht reichen")
        config["refill_windows"].append({"start": item, "end": end.strftime("%H:%M")})
    save_planner_config(config)
...
def public_notification_status() -> dict:
    try:
        return SMTPConfig.from_env().public_status()
    except ValueError as exc:
        return {"enabled": False, "configured": False, "configuration_error": str(exc)}


def home_assistant_diagnostics() -> dict:
    configured = bool(home_assistant_webhook_url() or home_assistant_refill_webhook_url())
    return {
        "configured": configured,
        "last_successful_contact_at": get_setting("home_assistant_last_success_at", ""),
        "last_error": get_setting("home_assistant_last_error", ""),
    }


def test_home_assistant_connection() -> dict:
    webhook = home_assistant_webhook_url() or home_assistant_refill_webhook_url()
    if not webhook:
        return {
    ...
    try:
        ...
        # Authentication errors still prove that Home Assistant answered. The
        # probe deliberately sends no secret and never invokes a webhook.
        if exc.code not in {401, 403, 404, 405}:
            message = f"Home Assistant antwortet mit HTTP {exc.code}."
            set_setting("home_assistant_last_error", message)
            record_home_assistant_health(False, message)
            return {**home_assistant_diagnostics(), "reachable": False, "error": message}
    ...
    set_setting("home_assistant_last_success_at", contacted_at)
    delete_setting("home_assistant_last_error")
    record_home_assistant_health(True, "Home Assistant ist erreichbar.")
    return {
        **home_assistant_diagnostics(),
        "reachable": True,
...
def get_state() -> dict:
    ...
    return {
        ...
        "weather_status": weather_diagnostics(),
        "notifications": public_notification_status(),
        "home_assistant": home_assistant_diagnostics(),
...
def watering_events(limit: int = 12) -> list[dict]:
    ...
    with connect() as conn:
        events = [
            ...
            for row in conn.execute(
                """
                SELECT id, ran_at, run_id, delivered_ml, actual_consumed_ml, temperature_c, rain_mm, source
        ...
        events.extend(
            ...
            for row in conn.execute(
                """
                SELECT id, ran_at, run_id, target_date, requested_ml, transferred_ml, duration_seconds, window_label, source
                FROM refill_events
                """
            )
        )
...
def _fetch_weather_uncached(
    ...
    planning_day = forecast[0] if forecast else {
        "date": local_now(str(timezone_name)).date().isoformat(),
        "temperature_c": 20.0,
        "rain_mm": 0.0,
        "wind_kmh": 0.0,
        "sunshine_hours": 0.0,
    ...
    result = {
        ...
        "longitude": longitude,
        "timezone": timezone_name,
        # Compatibility fields intentionally mirror the first complete daily
        # forecast. Current/hourly conditions are diagnostic-only below.
        ...
        "current": {
            ...
            "rain_mm": number_or_default(current.get("rain", current.get("precipitation")), 0),
...
def evaluate_weather(weather: dict, slot: str = "morning") -> dict:
    planning_weather = weather.get("planning_day") if weather.get("source") == "open-meteo" else weather
    ...
    return {
        ...
        "steps": [
            ...
            {"action": "HomeKit", "value": "Pumpe ausschalten"},
            {
                ...
                "body": {"auto_weather": True, "slot": "morning", "run_id": "{{run_id}}"},
            },
            {"action": "Ende Wenn"},
        ],
        "check_url": check_url,
        ...
        "manual_run_steps": [
            {"action": "URL", "value": manual_run_url},
            {
                ...
                "url": manual_run_url,
                "headers": {"Content-Type": "application/json"},
                "body": {"auto_weather": True, "run_id": "{{run_id}}"},
            },
        ],
    }
...
def optimize_routing(plants: list[dict], outlets: list[dict], max_cycles: int = 96) -> dict:
    ...
    for cycles in range(1, max_cycles + 1):
        ...
        candidate = {
            ...
            "optimization_method": candidate.get("optimization_method", "bounded_dynamic_programming"),
        }
...
def fallback_single_tube_plan(plants: list[dict], outlets: list[dict], max_cycles: int) -> dict:
    ...
    for plant in plants:
        ...
        option_sets.append(sorted(options, key=lambda item: item["score"]))
    ...
    if candidate is not None:
        return {"cycles": max_cycles, **candidate, "fallback": True}
...
def add_connection_comparison(plan: dict, plants: list[dict]) -> None:
    ...
    for assignment in plan["assignments"]:
        ...
        current_count = int(plant.get("current_tube_count", 0) or 0)
        recommended_count = sum(int(tube["count"]) for tube in assignment["tubes"])
        ...
        assignment["connection_action"] = (
            "add_hose" if recommended_count > current_count else "rewire"
            if abs(current_ml - recommended_ml) > tolerance else "none"
        )
        assignment["reason_codes"] = [
            "under_supply" if current_ml < recommended_ml else "over_supply",
            "additional_hose_needed" if recommended_count > current_count else "different_hose_combination",
        ] if abs(current_ml - recommended_ml) > tolerance else ["within_tolerance"]
...
def distributed_automation_windows(today: date, total_cycles: int, tzinfo) -> list[datetime]:
    config = planner_config()
    return configured_distributed_windows(
        today,
        total_cycles,
...
def automation_status(
    ...
    day_start = window_datetime(now.date(), config["watering_window_start"], now.tzinfo)
    day_end = window_datetime(now.date(), config["watering_window_end"], now.tzinfo)
...
def refill_status(balcony: dict) -> dict:
    ...
    schedule_times = [item["start"] for item in config["refill_windows"]]
    ...
    target_transfer_ml = (
        ...
        else math.ceil(requested_ml * float(config["refill_fraction"]))
    )
    ...
    if not next_window and config["refill_windows"]:
        next_window = window_datetime(
            now.date() + timedelta(days=1),
            config["refill_windows"][0]["start"],
            now.tzinfo,
        )
    last_event = latest_refill_event()
    last_event_at = parse_event_datetime(last_event.get("ran_at")) if last_event else None
    cooldown_until = (
        last_event_at.astimezone(now.tzinfo) + timedelta(minutes=int(config["refill_min_interval_minutes"]))
        if last_event_at
        else None
    )
...
def chronological_depletion_simulation(
    *,
    ...
    for day in projected_days:
        ...
        for window in config["refill_windows"]:
            refill_windows.append(
                (
                    window_datetime(day_value, window["start"], now.tzinfo),
                    window_datetime(day_value, window["end"], now.tzinfo),
                )
            )
    completed_refill_windows: set[str] = set()
    with connect() as conn:
        for row in conn.execute("SELECT target_date, window_label FROM refill_events"):
            try:
                completed_start = window_datetime(
    ...
    refill = result.get("refill", {})
    refill_tank = refill.get("refill_tank", {})
    latest_refill = latest_refill_event()
    return simulate_tanks(
        ...
        refill_current_ml=int(refill_tank.get("current_ml", balcony.get("refill_tank_current_ml", 0))),
        refill_capacity_ml=int(refill_tank.get("capacity_ml", balcony.get("refill_tank_capacity_ml", 0))),
        refill_enabled=bool(refill.get("enabled", refill_automation_enabled())),
        refill_pump_ml_per_min=int(refill.get("pump_ml_per_min", balcony.get("refill_pump_ml_per_min", 0))),
        refill_min_interval_minutes=int(config["refill_min_interval_minutes"]),
        ...
        last_refill_at=(
            parse_event_datetime(latest_refill.get("ran_at"))
            if latest_refill
            else None
        ),
    )
...
def projected_consumption_days(
    ...
    for weather in forecast_days:
        ...
        routing_plan = apply_fixed_connection_to_weather(
            configured_connection_plan(plants),
            calculated["plants"],
            outlets,
            max_cycles=int(planner_config()["max_cycles_per_day"]),
        )
...
def manual_run_status(result: dict) -> dict:
    delivered_per_cycle = int(result["pump"]["delivered_per_cycle_ml"])
    consumed_per_cycle = int(result["pump"].get("consumed_per_cycle_ml", delivered_per_cycle))
...
def manual_refill_plan(result: dict) -> dict:
    ...
    target_transfer_ml = (
        ...
        if config["refill_strategy"] == "target"
        else math.ceil(main_missing_ml * float(config["refill_fraction"]))
    )
...
def trigger_home_assistant_manual_run(result: dict, run_id: object = None) -> str:
    ...
    try:
        post_webhook(
            home_assistant_webhook_url(),
            {
                ...
                "consumed_per_cycle_ml": result["pump"]["consumed_per_cycle_ml"],
            },
            urlopen,
        )
    except ValueError as exc:
        record_home_assistant_health(False, str(exc))
        raise
    record_home_assistant_health(True, "Webhook ist wieder erreichbar.")
    return normalized_run_id


def trigger_home_assistant_manual_refill(result: dict, run_id: object = None) -> str:
    status = manual_refill_status(result)
    if not status["available"]:
        raise ValueError(status["reason"])
    normalized_run_id, _ = normalize_run_id(run_id, "refill")
    save_pending_refill_request({**status, "run_id": normalized_run_id})
    try:
        post_webhook(
        ...
        delete_setting("pending_refill_request")
        record_home_assistant_health(False, "Nachfuell-Webhook ist nicht erreichbar.")
        raise
    record_home_assistant_health(True, "Webhook ist wieder erreichbar.")
    return normalized_run_id
...
def record_home_assistant_health(reachable: bool, detail: str) -> None:
    try:
        notification_service().update_condition(
        ...
        # Notification failures must never hide the original webhook result.
        pass


def notification_conditions() -> list[dict]:
    state = get_state()
    balcony = state["balcony"]
    ...
    consumed_cycle_ml = calibrated_consumption_ml(nominal_cycle_ml)
    ...
    conditions = [
        notification_condition(
            ...
            f"Haupttank: {main_current} ml, kalibrierter Zyklusverbrauch: {consumed_cycle_ml} ml.",
        ),
        notification_condition(
            "refill_tank_low",
            refill_current <= 0 or refill_current / refill_capacity <= TANK_LOW_PERCENT / 100,
            "warning" if refill_current > 0 else "critical",
            "Vorratstank niedrig",
            f"Vorratstank: {refill_current} von {refill_capacity} ml.",
        ),
    ]
    refill = refill_status(balcony)
    ...
    try:
        ...
        first_unserved = parse_event_datetime(evaluation["depletion"].get("first_unserved_watering_at", ""))
        supply_days = (
            max(0, (first_unserved.astimezone(local_now(str(balcony["timezone_name"])).tzinfo).date() - local_now(str(balcony["timezone_name"])).date()).days)
            if first_unserved
            else WEATHER_FORECAST_DAYS
        )
        threshold = int(planner_config()["supply_warning_days"])
        conditions.append(
            notification_condition(
                "supply_range",
                ...
                f"Geschaetzte Reichweite: {supply_days} Tage, Warnschwelle: {threshold} Tage.",
            )
        )
        ...
        conditions.append(
            notification_condition(
                ...
                str(automation.get("summary", "Ein geplanter Lauf ist ueberfaellig.")),
            )
        )
    except ValueError:
        pass
    ...
    conditions.append(
        notification_condition(
            ...
            (
                f"Letzter erfolgreicher Abruf: {weather_status['last_successful_fetch_at'] or 'nie'}. "
...
def run_notification_check() -> list[dict]:
    service = notification_service()
    return [service.update_condition(**condition) for condition in notification_conditions()]
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/state":
            ...
            return
        if parsed.path == "/api/diagnostics/notifications":
            send_json(
                self,
                {
                    **notification_service().diagnostics(),
                    "weather": weather_diagnostics(),
                    "worker_running": bool(_notification_worker and _notification_worker.running),
                },
            )
            return
        if parsed.path == "/api/diagnostics/home-assistant":
            send_json(self, home_assistant_diagnostics())
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            if parsed.path == "/api/settings":
                config = save_planner_config(read_json(self))
                send_json(self, {"planner_config": config})
                return
            if parsed.path == "/api/notifications/test":
                send_json(self, notification_service().send_test())
                return
            if parsed.path == "/api/diagnostics/notifications/check":
                send_json(self, {"checks": run_notification_check(), **notification_service().diagnostics()})
                return
            if parsed.path == "/api/diagnostics/home-assistant/test":
                result = test_home_assistant_connection()
                send_json(
                    self,
                    result,
                    HTTPStatus.OK if result.get("reachable") else HTTPStatus.SERVICE_UNAVAILABLE,
                )
                return
            ...
            if parsed.path == "/api/homekit/mark-run":
                ...
                send_json(
                    self,
                    {
                        **evaluate_weather(weather, str(payload.get("slot", "morning"))),
                        "booking": booking,
                    },
                )
                return
            if parsed.path == "/api/manual-run":
                ...
                if not result["manual_run"]["available"]:
                    ...
                    return
                try:
                    run_id = trigger_home_assistant_manual_run(result, payload.get("run_id"))
                except ValueError as exc:
                    send_json(self, {"error": str(exc)}, HTTPStatus.BAD_GATEWAY)
                    return
                send_json(
            ...
            if parsed.path == "/api/manual-refill":
                ...
                if not result["manual_refill"]["available"]:
                    ...
                    return
                try:
                    run_id = trigger_home_assistant_manual_refill(result, payload.get("run_id"))
                except ValueError as exc:
                    send_json(self, {"error": str(exc)}, HTTPStatus.BAD_GATEWAY)
                    return
                send_json(
...
def save_balcony(payload: dict) -> None:
    ...
    if "planner_config" in payload:
        save_planner_config(payload["planner_config"])
...
def normalize_run_id(run_id: object, event_type: str) -> tuple[str, bool]:
    ...
    if len(value) > 128 or not re.fullmatch(r"[A-Za-z0-9._:-]+", value):
        raise ValueError("run_id darf maximal 128 Zeichen aus A-Z, a-z, 0-9, Punkt, Doppelpunkt, Unterstrich und Minus enthalten")
    return value, generated


def mark_refill_run(source: str = "home_assistant", run_id: object = None) -> dict:
    ...
    with connect() as conn:
        ...
        levels = conn.execute(
            """
            SELECT tank_capacity_ml, tank_current_ml, refill_tank_current_ml
            FROM balcony_settings WHERE id = 1
            """
        ).fetchone()
        transferred_ml = min(
            transferred_ml,
            max(0, int(levels["tank_capacity_ml"]) - int(levels["tank_current_ml"])),
            max(0, int(levels["refill_tank_current_ml"])),
        )
        if transferred_ml <= 0:
            raise ValueError("Zum Buchungszeitpunkt ist keine Nachfuellmenge mehr verfuegbar")
        conn.execute(
            """
            UPDATE balcony_settings
        ...
        conn.execute(
            ...
            INSERT INTO refill_events
                (ran_at, run_id, target_date, requested_ml, transferred_ml, duration_seconds, window_label, source)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                now,
...
def mark_run(
    ...
    normalized_run_id, legacy_generated = normalize_run_id(run_id, "watering")
    ...
    with connect() as conn:
        ...
        tank = conn.execute(
            "SELECT tank_current_ml FROM balcony_settings WHERE id = 1"
        ).fetchone()
        if tank is None or int(tank["tank_current_ml"]) < actual_consumed_ml:
            raise ValueError("Der Haupttank reicht nicht fuer den kalibrierten Verbrauch eines vollstaendigen Zyklus")
        cursor = conn.execute(
            """
            INSERT INTO watering_events
                (ran_at, run_id, delivered_ml, actual_consumed_ml, temperature_c, rain_mm, source)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                timestamp,
...
def start_notification_worker() -> NotificationWorker | None:
    ...
    disabled = str(os.environ.get("NOTIFICATION_WORKER_DISABLED", "")).lower() in {"1", "true", "yes", "on"}
    try:
...
def main() -> None:
    ...
    serve_http_api(
        ...
        port,
        AppHandler,
        on_start=start_notification_worker,
        on_stop=stop_notification_worker,
    )
