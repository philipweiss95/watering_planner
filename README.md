...
### Update von 1.5.6 auf 1.5.7

Version 1.5.7 kann direkt mit dem in 1.5.6 enthaltenen Updater installiert
...
Cache `watering-planner-1.5.7` geladen wird. Die Automatik kann anschließend
bei hohem Vorratstankstand auf eine Mindestnachfüllmenge warten.

### Update von 1.5.3 auf 1.5.4

Version 1.5.4 kann direkt mit dem in 1.5.3 enthaltenen Updater installiert
werden. Vorher wie gewohnt `data/watering.sqlite3` und `.env.synology` sichern.
Nach dem Update die iPhone-PWA vollständig schließen und neu öffnen, damit der
Cache `watering-planner-1.5.4` geladen wird. Künftige Installationen zeigen
ihren Fortschritt anschließend live auf der Update-Seite; die vier manuellen
Aktionen stehen auf der Startseite.

### Update von 1.5.2 auf 1.5.3

Version 1.5.3 kann direkt mit dem in 1.5.2 enthaltenen Updater installiert
werden. Vorher wie gewohnt `data/watering.sqlite3` und `.env.synology` sichern.
Nach dem Update die iPhone-PWA vollständig schließen und neu öffnen, damit der
Cache `watering-planner-1.5.3` geladen wird. Der Seitenstart nutzt anschließend
die zuletzt gespeicherten Wetterdaten; ein direkter Anbieterabruf erfolgt nur
noch über die explizite Wetteraktion.

### Update von 1.5.1 auf 1.5.2

Version 1.5.2 kann direkt mit dem in 1.5.1 enthaltenen Updater installiert
werden. Vorher wie gewohnt `data/watering.sqlite3` und `.env.synology` sichern.
Nach der Update-Prüfung zeigt die Info-Seite die Änderungen des Releases an.
Die iPhone-PWA nach der Installation vollständig schließen und neu öffnen,
damit der Cache `watering-planner-1.5.2` geladen wird.
