...
1.5.2, 1.5.3, 1.5.4, 1.5.5, 1.5.6 und 1.5.7 bauen direkt auf diesem Verlauf auf. Das
...
## Patch-Update von 1.5.6 auf 1.5.7

1. `data/watering.sqlite3` und `.env.synology` sichern.
2. Unter **Info > Updates** Version 1.5.7 suchen und den angezeigten Changelog
   prüfen.
3. Version 1.5.7 installieren und den erfolgreichen Containerwechsel
   abwarten.
...
1. `data/watering.sqlite3` und `.env.synology` sichern.
2. Unter **Info > Updates** Version 1.5.4 suchen und den angezeigten Changelog
   prüfen.
3. Version 1.5.4 installieren und den erfolgreichen Containerwechsel
   abwarten.
4. Die PWA vollständig schließen und neu öffnen.
5. Unter **Info > Updates** die neue Fortschrittsdarstellung und auf der
   Startseite alle vier manuellen Aktionen prüfen.

## Patch-Update von 1.5.2 auf 1.5.3

1. `data/watering.sqlite3` und `.env.synology` sichern.
2. Unter **Info > Updates** Version 1.5.3 suchen und den angezeigten Changelog
   prüfen.
3. Version 1.5.3 installieren und den erfolgreichen Containerwechsel
   abwarten.
4. Die PWA vollständig schließen und neu öffnen.
5. Gespeicherte Wetterdaten sowie die kompakten Ansichten unter **Verlauf**
   und **Pflanzen** prüfen.

## Patch-Update von 1.5.1 auf 1.5.2

1. `data/watering.sqlite3` und `.env.synology` sichern.
2. Unter **Info > Updates** Version 1.5.2 suchen und den angezeigten Changelog
   prüfen.
3. Version 1.5.2 installieren und den erfolgreichen Containerwechsel
   abwarten.
4. Die PWA vollständig schließen und neu öffnen.
5. **Info > Updates** sowie die mobilen Karten unter **System** prüfen.
...
6. Im Planner unter **System** zuerst den sicheren Home-Assistant-Test
   ausführen. Dieser prüft die konfigurierte Webhook-Adresse per `GET` und
   löst keinen Pumpen-Webhook aus.
...
2. Planner und Updater zeigen Version 1.5.7 beziehungsweise verwenden die
   Images mit Tag 1.5.7.
